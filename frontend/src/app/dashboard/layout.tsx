...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const menuItems = [
        ...
        { name: 'Pengaturan', href: '/dashboard/settings', icon: 'fa-cog', mobileLabel: 'Setting' },
    ];

    return (
        <div className="bg-[#fffbeb] text-gray-800 font-sans min-h-screen pb-20 md:pb-0 md:flex">
            {/* MOBILE BOTTOM NAV */}
            ...
            {/* DESKTOP SIDEBAR */}
            <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex-col h-screen fixed">
                ...
                <nav className="flex-1 p-4 space-y-2">
                    {menuItems.map(item => {
                        const isActive = pathname === item.href;
                        return (
                            <Link key={item.name} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition ${isActive ? 'bg-[#fffbeb] text-[#d97706] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-[#f59e0b]'}`}>
