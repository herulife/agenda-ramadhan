...
export default function ChildrenPage() {
    ...
    const handleAddChild = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            await api.post('/children', { name, gender, password });
            setName('');
            setPassword('');
            fetchChildren();
        } catch (err: any) {
            ...
            <form onSubmit={handleAddChild} className="bg-amber-50 border-[3px] border-white rounded-[30px] p-5 mb-8 shadow-[0_8px_0_#b8772e] max-w-2xl">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                    ...
                    <div className="w-full md:w-32">
                        <label className="block text-sm font-semibold text-amber-800 mb-1">Gender</label>
                        <select
                            value={gender}
                            onChange={(e) => setGender(e.target.value)}
                            className="w-full p-2.5 border-2 border-amber-200 rounded-xl focus:border-orange-400 focus:outline-none bg-white"
                        >
                    ...
                    <div className="flex-1">
                        <label className="block text-sm font-semibold text-amber-800 mb-1">Password (Opsional)</label>
                        <input
                            type="password"
                            value={password}
                            onChange={(e) => setPassword(e.target.value)}
                            className="w-full p-2.5 border-2 border-amber-200 rounded-xl focus:border-orange-400 focus:outline-none"
                            placeholder="Untuk login anak"
                        />
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    children.map(child => (
                        <div key={child.ID} className="bg-white border-[3px] border-amber-100 rounded-[30px] p-5 flex items-center gap-4 shadow-[0_6px_0_#fde68a]">
                            <div className="text-4xl bg-orange-100 w-16 h-16 rounded-full flex items-center justify-center shadow-sm">
                                {child.gender === 'male' ? '👦' : '🧕'}
                            </div>
                            <div className="flex-1">
